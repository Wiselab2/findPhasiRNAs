...
def mapSmallRNAReads():
    ...
    location_of_files="/work/LAS/rpwise-lab/sagnik/small_rna/data/PARE_raw_data/"
    for inputfile in all_input_files:
        cmd="nohup /work/LAS/rpwise-lab/sagnik/softwares/bowtie-1.2.2-linux-x86_64/bowtie --threads 5 "
        cmd+=" -f -v 0 -l 15 -e 99999 -a -S --large-index --no-unal "
...
def calculatePercentageOfMappedReads(genome=1):
    all_input_files=["Hv_pool1_adapter_trimmed.fastq","Hv_pool2_adapter_trimmed.fastq","Hv_pool3_adapter_trimmed.fastq","Hv_pool4_adapter_trimmed.fastq","Hv_pool5_adapter_trimmed.fastq"]
    ...
    location_of_files="/work/LAS/rpwise-lab/sagnik/small_rna/data/PARE_raw_data/"
    for i in range(5):
        cmd="wc -l "+location_of_files+all_input_files[i]+" > temp"
        os.system(cmd)
        total_reads=int(open("temp").read().strip().split()[0])//4
        
        reads_min_length_15=0
        fhr=open(location_of_files+all_input_files_unique[i],"r")
        for line in fhr:
            if ">" in line:
        ...
        if genome==1:
            ...
            cmd="grep -v ^\"@\" "+location_of_files+all_input_files_unique[i].split(".fasta")[0]+"_transcriptome_bowtie1.sam"+"|cut -f1|uniq|cut -d\"_\" -f2|awk '{sum += $1} END {print sum}' > temp"
        os.system(cmd)
        ...
        if genome==0:
            ...
            fhw=open(location_of_files+all_input_files_unique[i].split(".fasta")[0]+"_bowtie1.error","w")
...
def mapReadsToTranscriptome():
    ...
    for inputfile in all_input_files:
        cmd="nohup /work/LAS/rpwise-lab/sagnik/softwares/bowtie-1.2.2-linux-x86_64/bowtie --threads 5 "
        cmd+=" -f -v 0 -l 15 -e 99999 -a -S --no-unal "
...
def main():
    ...
    #mapSmallRNAReads()
    calculatePercentageOfMappedReads(1)
    mapReadsToTranscriptome()
    calculatePercentageOfMappedReads(0)
